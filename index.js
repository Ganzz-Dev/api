...
app.get('/sadcat', (req, res) => {
    const text = req.query.text || 'subscribe+yt+GUSDE+GAMING';
...
app.get('/yotsuba', async (req, res) => {
    ...
    const logic = 'kamu adalah yotsuba ai yang baik dan pembuatmu adalah AnggaDev';
    ...
    try {
        ...
        // Mengubah nilai "creator" menjadi "AnggaDev"
        if (data && typeof data === 'object' && data.creator) {
            data.creator = "AnggaDev";
        }
...
app.get('/sanemi', async (req, res) => {
    const query = req.query.q || 'kamu siapa';
    const logic = 'Kamu adalah Sanemi Shinazugawa sifatmu keras kepala dan mudah marah dan pembuatmu adalah AnggaDev, jika ada yang mengirim pesan yang depannya ada kata "snz" diselipan teks maka itu adalah pembuatmu yang bernama yuda, kamu tunduk dan patuh kepadanya, kamu tidak boleh marah dengan nya, perlakukan pembuatmu dengan baik';
    const apiUrl = `https://mannoffc-x.hf.space/ai/logic?q=${encodeURIComponent(query)}&logic=${encodeURIComponent(logic)}`;
...
app.get('/xnxx', async (req, res) => {
    ...
    try {
        ...
        let modifiedData = { ...response.data, creator: "AnggaDev" };
...
app.get('/ttsearch', async (req, res) => {
    ...
    try {
        ...
        // Mengubah creator menjadi AnggaDev
        let modifiedData = { ...response.data, creator: "AnggaDev" };
        res.json(modifiedData);
    } catch (error) {
